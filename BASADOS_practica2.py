...
max_samples_per_transfer = max_bytes_per_transfer/2 #this is because each sample is of 16bits
...
for n_block in range(0,complete_transfers-1):
    for sample in range(0,max_samples_per_transfer-1):
        Mb[n_block,sample] = data[((n_block)*750)+sample]        
    #end
#end

number_of_bytes_final_transfer = int(number_of_bytes_final_transfer)
for sample in range(0,(number_of_bytes_final_transfer/2) - 1):
    Mb[n_block,sample] = data[((n_block+1)*750)+sample]
#end
